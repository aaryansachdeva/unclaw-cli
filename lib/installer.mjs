...
// Claude Code skill (~/.claude/skills/unclaw/SKILL.md). Claude Code's native
// user-facing extension point is a "skill" (shows in /skills, invokable as
// /unclaw) , which an MCP server alone does not provide. So for Claude we
// install BOTH: the MCP `speak` tool (voicing) AND this skill (the launch +
// talkative-rhythm playbook, matching the /unclaw command the other agents get).
// The command examples call the installed shim by ABSOLUTE node + path so they
// work regardless of PATH; the MCP tools are the preferred route when loaded.
export function claudeSkillMarkdown(node, runtime) {
  ...
  return [
    ...
    'Give the app a few seconds, then confirm it is live with the `unclaw_status`',
